...
const InputField = ({ label, id, error, ...props }: {label: string, id: string, error?: string, [key: string]: any}) => (
    <div className="mb-3 sm:mb-4">
        <label htmlFor={id} className="inline-block bg-orange-400 text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-t-md -mb-px">{label}</label>
        <input id={id} className={`w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm sm:text-base border ${error ? 'border-red-500' : 'border-teal-500'} rounded-b-md rounded-r-md focus:outline-none focus:ring-1 ${error ? 'focus:ring-red-500' : 'focus:ring-teal-500'}`} {...props} />
...
const TextareaField = ({ label, id, ...props }: {label: string, id: string, [key: string]: any}) => (
    <div className="mb-3 sm:mb-4">
        <label htmlFor={id} className="inline-block bg-orange-400 text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-t-md -mb-px">{label}</label>
        <textarea id={id} rows={3} className="w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm sm:text-base border border-teal-500 rounded-b-md rounded-r-md focus:outline-none focus:ring-1 focus:ring-teal-500" {...props}></textarea>
    </div>
);
...
const VariationFields: React.FC<{
    ...
    return (
        <div className="p-2 sm:p-3 border rounded-md relative mb-2 sm:mb-3 bg-gray-50 border-teal-300">
            <div className="flex flex-col sm:grid sm:grid-cols-1 md:grid-cols-8 gap-2 sm:gap-3">
                 <div className="md:col-span-4">
                    <label className="text-xs sm:text-sm font-medium text-gray-600 block mb-1">Nama Variasi</label>
                    <input 
                        type="text" 
                        placeholder="e.g., Pedas" 
                        value={variation.name}
                        onChange={(e) => updateVariation(index, 'name', e.target.value)}
                        className="w-full px-2 sm:px-3 py-1.5 sm:py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-teal-500"
                    />
                </div>
                 <div className="md:col-span-2">
                    <label className="text-xs sm:text-sm font-medium text-gray-600 block mb-1">Harga</label>
                    <input 
                        type="number" 
                 ...
                 <div className="md:col-span-1">
                    <label className="text-xs sm:text-sm font-medium text-gray-600 block mb-1">Stok</label>
                    <input 
                        type="number" 
                        placeholder="e.g., 50" 
                ...
                <div className="md:col-span-1 flex justify-end md:items-end md:pb-2">
                     <button
                        type="button"
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Tambah Produk Baru">
            <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3 sm:mb-4">
                    <label className="inline-block bg-orange-400 text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-t-md -mb-px">Variasi Produk (Opsional)</label>
                    <div className="p-2 sm:p-4 border border-teal-500 rounded-b-md rounded-r-md space-y-2">
                         {variations.map((v, i) => (
                        ...
                        <button
                            ...
                            className="flex items-center text-orange-500 font-medium text-xs sm:text-sm hover:text-orange-600 transition-colors"
                        >
                ...
                <button type="submit" className="w-full bg-orange-400 text-white py-2.5 sm:py-3 rounded-lg text-sm sm:text-base font-bold hover:bg-orange-500 transition-colors">
